...
public:
    ...
    /// @brief アロケータを取得
    memory::IAllocator *allocator() const
    {
        return allocator_;
    }
    ...
    RenderResponse &acquireResponse()
    {
        ...
        FLEXIMG_DEBUG_WARN("ERROR: RenderResponse pool exhausted! MAX=%d", MAX_RESPONSES);
    ...
    void releaseResponse(RenderResponse &resp)
    {
        ...
        if (idx < MAX_RESPONSES) {
            if (!resp.inUse) {
                FLEXIMG_DEBUG_WARN("WARN: releaseResponse called on non-inUse response idx=%d", static_cast<int>(idx));
            }
    ...
    void resetScanlineResources()
    {
        ...
        // 1つは下流に渡されるため、1以下なら正常
        {
            ...
            if (inUseCount > 1) {
                FLEXIMG_DEBUG_WARN("WARN: resetScanlineResources with %d responses still in use", inUseCount);
            }
        }
#endif
        for (uint_fast8_t i = 0; i < MAX_RESPONSES; ++i) {
