...
public:
  ...
  Node(Node &&other) noexcept
      : inputs_(std::move(other.inputs_)), outputs_(std::move(other.outputs_)),
        context_(nullptr) {
    prepareResponse_.status = PrepareStatus::Idle;
    // ownerポインタを自分に修正
  ...
  bool connectFrom(Node &source, int sourceOutputIndex = 0,
                   int inputIndex = 0) {
  ...
  // 準備処理（スクリーン情報を受け取る）
  virtual void prepare(const RenderRequest &screenInfo) { (void)screenInfo; }
  ...
  virtual RenderResponse &pullProcess(const RenderRequest &request) final {
    // 共通処理: スキャンライン処理チェック
    FLEXIMG_ASSERT(request.height == 1,
                   "Scanline processing requires height == 1");
    // 共通処理: 準備完了状態チェック
  ...
  void
  consolidateIfNeeded(RenderResponse &input,
...
protected:
  ...
  virtual void onPushProcess(RenderResponse &input,
                             const RenderRequest &request) {
  ...
  ImageBuffer
  convertFormat(ImageBuffer &&buffer, PixelFormatID target,
...
ImageBuffer Node::convertFormat(ImageBuffer &&buffer, PixelFormatID target,
  ...
  ImageBuffer result =
      std::move(buffer).toFormat(target, mode, newAlloc, converter);
...
RenderResponse &Node::makeResponse(ImageBuffer &&buf, Point origin) {
  FLEXIMG_ASSERT(context_ != nullptr,
                 "RenderContext required for makeResponse");
...
RenderResponse &Node::makeEmptyResponse(Point origin) {
  FLEXIMG_ASSERT(context_ != nullptr,
                 "RenderContext required for makeEmptyResponse");
  RenderResponse &resp = context_->acquireResponse();
  resp.origin = origin;
  return resp;
}
