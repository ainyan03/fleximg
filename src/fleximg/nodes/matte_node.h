...
public:
  ...
  static void benchProcessRowWithFg(uint8_t *d, const uint8_t *m,
                                    const uint8_t *s, int pixelCount);
...
private:
  ...
  struct InputView {
    ...
    const uint8_t *rowAt(int_fast16_t y) const {
      ...
      if (static_cast<unsigned>(srcY) >= static_cast<unsigned>(height))
        return nullptr;
      return ptr + srcY * stride;
    }

    // RenderResponseから構築
    static InputView from(const RenderResponse &resp, int_fixed outOriginX,
                          int_fixed outOriginY) {
      InputView v;
      if (!resp.isValid())
        return v;
      ViewPort vp = resp.view();
      v.ptr = static_cast<const uint8_t *>(vp.data) + vp.y * vp.stride +
  ...
  // 戻り値: 有効範囲の幅（0なら全面0）
  static int_fast16_t scanMaskZeroRanges(const uint8_t *maskData,
                                         int_fast16_t maskWidth,
                                         int_fast16_t &outLeftSkip,
...
DataRange MatteNode::calcUpstreamRanges(const RenderRequest &request) const {
  ...
  rangeCache_.maskRange =
      maskNode ? maskNode->getDataRange(request) : DataRange{};
  ...
  if (rangeCache_.maskRange.hasData() && rangeCache_.fgRange.hasData()) {
    int16_t intersectStart =
        std::max(rangeCache_.maskRange.startX, rangeCache_.fgRange.startX);
    int16_t intersectEnd =
        std::min(rangeCache_.maskRange.endX, rangeCache_.fgRange.endX);
    if (intersectStart < intersectEnd) {
      if (intersectStart < startX)
        startX = intersectStart;
      if (intersectEnd > endX)
        endX = intersectEnd;
    }
  }

  rangeCache_.unionRange =
      (startX < endX) ? DataRange{startX, endX} : DataRange{};
...
DataRange MatteNode::getDataRange(const RenderRequest &request) const {
  ...
  if (rangeCache_.valid && rangeCache_.origin.x == request.origin.x &&
...
RenderResponse &MatteNode::onPullProcess(const RenderRequest &request) {
...

  {
    ...
    if (rangeCache_.fgRange.hasData()) {
      if (rangeCache_.fgRange.startX < fgBgStart)
        fgBgStart = rangeCache_.fgRange.startX;
      if (rangeCache_.fgRange.endX > fgBgEnd)
        fgBgEnd = rangeCache_.fgRange.endX;
    }
    if (rangeCache_.bgRange.hasData()) {
      if (rangeCache_.bgRange.startX < fgBgStart)
        fgBgStart = rangeCache_.bgRange.startX;
      if (rangeCache_.bgRange.endX > fgBgEnd)
        fgBgEnd = rangeCache_.bgRange.endX;
    }
    ...
    auto unionWidth =
        static_cast<int_fast16_t>(from_fixed(unionMaxX - unionMinX));
    auto unionHeight =
        static_cast<int_fast16_t>(from_fixed(unionMaxY - unionMinY));
    ...
    ImageBuffer outputBuf(unionWidth, unionHeight,
    ...
    // bgがあればコピー
    if (bgResultPtr) {
      auto bgOffsetX = static_cast<int_fast16_t>(
          from_fixed(bgResultPtr->origin.x - unionMinX));
      auto bgOffsetY = static_cast<int_fast16_t>(
          from_fixed(bgResultPtr->origin.y - unionMinY));
      ...
      if (converter) {
        ...
        auto srcBytesPerPixel =
            static_cast<int_fast16_t>(bgViewPort.bytesPerPixel());

        // bgの有効範囲を計算（出力座標系）
        auto copyStartX = std::max<int_fast16_t>(0, bgOffsetX);
        auto copyEndX =
            std::min<int_fast16_t>(unionWidth, bgOffsetX + bgViewPort.width);
        auto copyStartY = std::max<int_fast16_t>(0, bgOffsetY);
        auto copyEndY =
            std::min<int_fast16_t>(unionHeight, bgOffsetY + bgViewPort.height);
        ...
        if (copyWidth > 0) {
          ...
          for (auto y = copyStartY; y < copyEndY; ++y) {
            ...
            const uint8_t *srcRow =
                static_cast<const uint8_t *>(bgViewPort.data) +
    ...
    InputView fgView = InputView::from(fgResultPtr ? *fgResultPtr : emptyResult,
                                       unionMinX, unionMinY);
...
int_fast16_t MatteNode::scanMaskZeroRanges(const uint8_t *maskData,
                                           int_fast16_t maskWidth,
                                           int_fast16_t &outLeftSkip,
                                           int_fast16_t &outRightSkip) {
  ...
  int_fast16_t leftSkip = 0;
  {
    ...
    {
      const uint32_t *p32 =
          reinterpret_cast<const uint32_t *>(maskData + leftSkip);
      const uint32_t *p32_end = p32 + ((maskWidth - leftSkip) >> 2);
      while (p32 < p32_end && *p32 == 0) {
        ++p32;
      }
      leftSkip = static_cast<int_fast16_t>(
          reinterpret_cast<const uint8_t *>(p32) - maskData);
    }
...
scan_right:
  ...
  int_fast16_t rightSkip = 0;
  {
    ...
    {
      const uint32_t *p32 =
          reinterpret_cast<const uint32_t *>(maskData + maskWidth - rightSkip) -
          1;
      const uint32_t *p32_end = p32 - ((limit - rightSkip) >> 2);
      while (p32 > p32_end && *p32 == 0) {
        --p32;
      }
      rightSkip = static_cast<int_fast16_t>(
          maskData + maskWidth - reinterpret_cast<const uint8_t *>(p32 + 1));
    }
...
// ----------------------------------------------------------------------------
static inline void processRowNoFg(uint8_t *__restrict__ d,
...
// ----------------------------------------------------------------------------
static inline void processRowWithFg(uint8_t *__restrict__ d,
                                    const uint8_t *__restrict__ m,
...
void MatteNode::applyMatteOverlay(ImageBuffer &output, int_fast16_t outWidth,
  ...
  const auto maskXEnd =
      std::min<int_fast16_t>(outWidth, mask.width + mask.offsetX);
  if (maskXStart >= maskXEnd)
    return;

  const auto maskSrcOffsetX =
      static_cast<int_fast16_t>(maskXStart - mask.offsetX);

  // 前景の有効X範囲（事前計算）
  const auto fgXStart =
      fg.valid() ? std::max<int_fast16_t>(maskXStart, fg.offsetX) : maskXEnd;
  const auto fgXEnd =
      fg.valid() ? std::min<int_fast16_t>(maskXEnd, fg.width + fg.offsetX)
                 : maskXStart;
  const auto fgSrcOffsetX = static_cast<int_fast16_t>(fgXStart - fg.offsetX);

  // 3領域の幅を事前計算
  const auto leftWidth =
      static_cast<int_fast16_t>(fgXStart - maskXStart); // 左領域（fgなし）
  const auto midWidth =
      static_cast<int_fast16_t>(fgXEnd - fgXStart); // 中央領域（fg/bg両方）
  const auto rightWidth =
      static_cast<int_fast16_t>(maskXEnd - fgXEnd); // 右領域（fgなし）
  ...
  for (int_fast16_t y = 0; y < outHeight; ++y) {
    ...
    if (midWidth > 0) {
      ...
      if (fgRowBase) {
        processRowWithFg(dBase + leftWidth * 4, mBase + leftWidth,
    ...
    // 右領域: fgなし
    if (rightWidth > 0) {
      processRowNoFg(dBase + (leftWidth + midWidth) * 4,
...
void MatteNode::benchProcessRowWithFg(uint8_t *d, const uint8_t *m,
                                      const uint8_t *s, int pixelCount) {
  processRowWithFg(d, m, s, static_cast<int_fast16_t>(pixelCount));
}

void MatteNode::benchProcessRowNoFg(uint8_t *d, const uint8_t *m,
                                    int pixelCount) {
