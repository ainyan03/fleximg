...
public:
  ...
  void setupWithBounds(const ViewPort &image, int_fast16_t left,
                       int_fast16_t top, int_fast16_t right,
                       int_fast16_t bottom) {
    source_ = image;
  ...
  void setupFromNinePatch(const ViewPort &ninePatchImage) {
    if (!ninePatchImage.isValid() || ninePatchImage.width < 3 ||
        ninePatchImage.height < 3) {
      sourceValid_ = false;
      return;
    }
    ...
    auto isBlack = [&](int x, int y) -> bool {
      const uint8_t *pixel =
          static_cast<const uint8_t *>(ninePatchImage.pixelAt(x, y));
      if (!pixel)
        return false;
    ...
    // 内部画像（外周1pxを除く）を抽出
    ViewPort innerImage =
        view_ops::subView(ninePatchImage, 1, 1, ninePatchImage.width - 2,
                          ninePatchImage.height - 2);
    ...
    // 境界座標を計算
    auto left =
        static_cast<int_fast16_t>((stretchXStart >= 0) ? (stretchXStart) : 0);
    auto right = static_cast<int_fast16_t>(
        (stretchXEnd >= 0) ? (innerImage.width - 1 - stretchXEnd) : 0);
    auto top =
        static_cast<int_fast16_t>((stretchYStart >= 0) ? (stretchYStart) : 0);
    auto bottom = static_cast<int_fast16_t>(
        (stretchYEnd >= 0) ? (innerImage.height - 1 - stretchYEnd) : 0);
...
private:
  ...
  // 1軸方向のクリッピング計算（横/縦共通）
  void calcAxisClipping(float outputSize, int_fast16_t srcFixed0,
                        int_fast16_t srcFixed2, float &outWidth0,
                        float &outWidth1, float &outWidth2, int16_t &effSrc0,
                        int16_t &effSrc2);
...
PrepareResponse
NinePatchSourceNode::onPullPrepare(const PrepareRequest &request) {
  // ジオメトリ計算（まだなら）
  if (!geometryValid_) {
    updatePatchGeometry();
  }
  ...
  if (hasCombinedAffine) {
    ...
    float transformedPosX =
        matrixWithPos.a * positionX_ + matrixWithPos.b * positionY_;
    float transformedPosY =
        matrixWithPos.c * positionX_ + matrixWithPos.d * positionY_;
    matrixWithPos.tx += transformedPosX;
    matrixWithPos.ty += transformedPosY;

    // 出力矩形に順変換を適用して出力側のAABBを計算
    calcAffineAABB(static_cast<float>(outputWidth_),
                   static_cast<float>(outputHeight_), {pivotX_, pivotY_},
...
RenderResponse &
NinePatchSourceNode::onPullProcess(const RenderRequest &request) {
  ...
  for (auto i : drawOrder) {
    ...
    if (range.hasData()) {
      if (range.startX < canvasStartX)
        canvasStartX = range.startX;
      if (range.endX > canvasEndX)
  ...
  ImageBuffer canvasBuf = canvas_utils::createCanvas(
      canvasWidth, request.height, InitPolicy::Zero, allocator());
  ...
  for (auto i : drawOrder) {
    ...
    canvas_utils::placeFirst(canvasView, canvasOriginX, canvasOriginY,
                             patchResult.view(), patchResult.origin.x,
  ...
  return makeResponse(std::move(canvasBuf),
                      Point{canvasOriginX, canvasOriginY});
}

DataRange
NinePatchSourceNode::getDataRange(const RenderRequest &request) const {
  if (!sourceValid_ || outputWidth_ <= 0 || outputHeight_ <= 0) {
...
void NinePatchSourceNode::calcAxisClipping(float outputSize,
                                           int_fast16_t srcFixed0,
                                           int_fast16_t srcFixed2,
                                           float &outWidth0, float &outWidth1,
                                           float &outWidth2, int16_t &effSrc0,
                                           int16_t &effSrc2) {
  // ソースサイズは常に元のまま
...
void NinePatchSourceNode::updatePatchGeometry() {
  ...
  // 横方向・縦方向のクリッピング計算
  calcAxisClipping(outputWidth_, srcLeft_, srcRight_, patchWidths_[0],
                   patchWidths_[1], patchWidths_[2], effectiveSrcLeft_,
                   effectiveSrcRight_);
  calcAxisClipping(outputHeight_, srcTop_, srcBottom_, patchHeights_[0],
                   patchHeights_[1], patchHeights_[2], effectiveSrcTop_,
                   effectiveSrcBottom_);
  ...
  int16_t srcX[3] = {0, srcLeft_,
                     static_cast<int16_t>(source_.width - effectiveSrcRight_)};
  int16_t srcY[3] = {
      0, srcTop_, static_cast<int16_t>(source_.height - effectiveSrcBottom_)};
  ...
  for (int row = 0; row < 3; row++) {
    for (int col = 0; col < 3; col++) {
      ...
      if (effW[col] > 0 && effH[row] > 0) {
        ViewPort subView =
            view_ops::subView(source_, srcX[col] + dx, srcY[row] + dy,
                              effW[col] + dw, effH[row] + dh);
        ...
        if (row == 0)
          edgeFade |= EdgeFade_Top; // 上端パッチ: 上辺フェード有効
        if (row == 2)
          edgeFade |= EdgeFade_Bottom; // 下端パッチ: 下辺フェード有効
        if (col == 0)
          edgeFade |= EdgeFade_Left; // 左端パッチ: 左辺フェード有効
        if (col == 2)
          edgeFade |= EdgeFade_Right; // 右端パッチ: 右辺フェード有効
      ...
      // 平行移動量
      float tx =
          patchOffsetX_[col] + static_cast<float>(dx) - pivotXf + positionX_;
      float ty =
          patchOffsetY_[row] + static_cast<float>(dy) - pivotYf + positionY_;
