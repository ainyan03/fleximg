...
public:
  ...
  void setPosition(float x, float y) { setTranslation(x, y); }
...
private:
  ...
  int_fixed pivotX_ =
      0; // 画像内の基準点X（pivot: 回転・配置の中心、固定小数点 Q16.16）
  int_fixed pivotY_ =
      0; // 画像内の基準点Y（pivot: 回転・配置の中心、固定小数点 Q16.16）
  ...
  int_fixed baseTxWithOffsets_ =
      0; // 事前計算統合: invTx + srcPivot + rowOffset + dxOffset
  int_fixed baseTyWithOffsets_ =
      0; // 事前計算統合: invTy + srcPivot + rowOffset + dxOffset
  ...
  bool calcScanlineRange(const RenderRequest &request, int32_t &dxStart,
                         int32_t &dxEnd, int32_t *baseXWithHalf = nullptr,
...
PrepareResponse SourceNode::onPullPrepare(const PrepareRequest &request) {
  ...
  if (affine_.isValid()) {
    ...
    const int32_t prepareOffsetX =
        static_cast<int32_t>((static_cast<int64_t>(prepareOriginX_) * invA +
                              static_cast<int64_t>(prepareOriginY_) * invB) >>
                             INT_FIXED_SHIFT);
    const int32_t prepareOffsetY =
        static_cast<int32_t>((static_cast<int64_t>(prepareOriginX_) * invC +
                              static_cast<int64_t>(prepareOriginY_) * invD) >>
                             INT_FIXED_SHIFT);
    ...
    const bool useBilinear =
        ...
        source_.formatID && source_.formatID->copyQuadDDA;
    ...
    baseTxWithOffsets_ = affine_.invTxFixed + srcPivotXFixed16 +
                         affine_.rowOffsetX + affine_.dxOffsetX +
                         prepareOffsetX;
    baseTyWithOffsets_ = affine_.invTyFixed + srcPivotYFixed16 +
    ...
    bool isTranslationOnly =
        !useBilinear_ && invA == one && invD == one && invB == 0 && invC == 0;
  ...
  calcAffineAABB(aabbWidth, aabbHeight, {aabbPivotX, aabbPivotY},
                 combinedMatrix, result.width, result.height, result.origin);
...
RenderResponse &SourceNode::onPullProcess(const RenderRequest &request) {
  ...
  const int32_t baseX = baseTxWithOffsets_ + deltaX * affine_.invMatrix.a +
                        deltaY * affine_.invMatrix.b;
  const int32_t baseY = baseTyWithOffsets_ + deltaX * affine_.invMatrix.c +
                        deltaY * affine_.invMatrix.d;
  ...
  Point adjustedOrigin = {request.origin.x + to_fixed(dxStartX),
...
RenderResponse &
SourceNode::pullProcessWithAffine(const RenderRequest &request) {
  // スキャンライン有効範囲を計算
  ...
  Point adjustedOrigin = {request.origin.x + to_fixed(dxStart),
                          request.origin.y};
  ...
  if (useBilinear_) {
    outFormat =
        view_ops::canUseSingleChannelBilinear(source_.formatID, edgeFadeFlags_)
            ? source_.formatID
  ...
  ImageBuffer *output =
      resp.createBuffer(validWidth, 1, outFormat, InitPolicy::Uninitialized);
  ...
  if (useBilinear_) {
    ...
    const PixelAuxInfo *auxPtr =
        (auxInfo.palette || auxInfo.colorKeyRGBA8 != auxInfo.colorKeyReplace)
            ? &auxInfo
            : nullptr;
    view_ops::copyRowDDABilinear(
        dstRow, source_, validWidth, srcX_fixed + offsetX - halfPixel,
    ...
    if (source_.formatID && source_.formatID->copyRowDDA) {
      source_.formatID->copyRowDDA(static_cast<uint8_t *>(dstRow),
