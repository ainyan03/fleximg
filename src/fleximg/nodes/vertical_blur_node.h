...
public:
  VerticalBlurNode() { initPorts(1, 1); }
...
private:
  ...
  int_fixed upstreamOriginX_ =
      0; // 上流pullProcessのorigin.x（radius=0と同じ出力用）
  ...
  // 内部実装（宣言のみ）
  RenderResponse &pullProcessPipeline(Node *upstream,
                                      const RenderRequest &request);
  void updateStageCache(int_fast16_t stageIndex, Node *upstream,
                        const RenderRequest &request, int_fast16_t newY);
  void fetchRowToStageCache(BlurStage &stage, Node *upstream,
                            const RenderRequest &request, int_fast16_t srcY,
                            int_fast16_t cacheIndex);
  void fetchRowFromPrevStage(int_fast16_t stageIndex, Node *upstream,
                             const RenderRequest &request, int_fast16_t srcY,
                             int_fast16_t cacheIndex);
  void updateStageColSum(BlurStage &stage, int_fast16_t cacheIndex, bool add);
  void computeStageOutputRow(BlurStage &stage, ImageBuffer &output,
                             int_fast16_t width);
  ...
  void storeInputRowToStageCache(BlurStage &stage, const ImageBuffer &input,
                                 int_fast16_t cacheIndex,
...
void VerticalBlurNode::prepare(const RenderRequest &screenInfo) {
  ...
  size_t cacheBytes = static_cast<size_t>(passes_) *
                      (static_cast<size_t>(kernelSize()) *
                           static_cast<size_t>(cacheWidth_) * 4 +
...
DataRange VerticalBlurNode::getDataRange(const RenderRequest &request) const {
  ...
  for (auto dy = static_cast<int_fast16_t>(-expansion); dy <= expansion; ++dy) {
    ...
    if (rowRange.hasData()) {
      if (rowRange.startX < startX)
        startX = rowRange.startX;
      if (rowRange.endX > endX)
        endX = rowRange.endX;
    }
  }
...
PrepareResponse VerticalBlurNode::onPullPrepare(const PrepareRequest &request) {
  ...
  int_fast16_t expansion = radius_ * passes_;
  upstreamResult.height =
      static_cast<int16_t>(upstreamResult.height + expansion * 2);
...
void VerticalBlurNode::onPushProcess(RenderResponse &input,
                                     const RenderRequest &request) {
  // radius=0の場合はスルー
  if (radius_ == 0) {
    Node *downstream = downstreamNode(0);
  ...
  if (!input.isValid()) {
    std::memset(stage0.rowCache[static_cast<size_t>(slot0)].view().data, 0,
                static_cast<size_t>(cacheWidth_) * 4);
  } else {
    // バッファ準備
    ...
    int_fast16_t xOffset =
        static_cast<int_fast16_t>(from_fixed(inputOrigin.x - baseOriginX_));
...
RenderResponse &
VerticalBlurNode::pullProcessPipeline(Node *upstream,
                                      const RenderRequest &request) {
  int_fast16_t requestY =
      static_cast<int_fast16_t>(from_fixed(request.origin.y));
  ...
  int_fast16_t srcStartX =
      static_cast<int_fast16_t>(from_fixed_floor(interLeft - cacheLeft));
  int_fast16_t srcEndX =
      static_cast<int_fast16_t>(from_fixed_ceil(interRight - cacheLeft));
  ...
  for (int_fast16_t cacheX = srcStartX; cacheX < srcEndX; cacheX++) {
    ...
    if (lastStage.colSumA[static_cast<size_t>(cacheX)] > 0) {
      ...
      outRow[outOff] =
          static_cast<uint8_t>(lastStage.colSumR[cx] / lastStage.colSumA[cx]);
      outRow[outOff + 1] =
          static_cast<uint8_t>(lastStage.colSumG[cx] / lastStage.colSumA[cx]);
      outRow[outOff + 2] =
          static_cast<uint8_t>(lastStage.colSumB[cx] / lastStage.colSumA[cx]);
      outRow[outOff + 3] = static_cast<uint8_t>(lastStage.colSumA[cx] /
                                                static_cast<uint32_t>(ks));
    } else {
      outRow[outOff] = outRow[outOff + 1] = outRow[outOff + 2] =
          outRow[outOff + 3] = 0;
    }
  }
...
void VerticalBlurNode::updateStageCache(int_fast16_t stageIndex, Node *upstream,
                                        const RenderRequest &request,
                                        int_fast16_t newY) {
  ...
  if (stage.currentY == newY)
    return;

  int_fast16_t step = (stage.currentY < newY) ? 1 : -1;

  while (stage.currentY != newY) {
    int_fast16_t newSrcY =
        static_cast<int_fast16_t>(stage.currentY + step * (radius_ + 1));
...
void VerticalBlurNode::fetchRowToStageCache(BlurStage &stage, Node *upstream,
                                            const RenderRequest &request,
                                            int_fast16_t srcY,
                                            int_fast16_t cacheIndex) {
  ...
  int_fast16_t srcOffsetX =
      static_cast<int_fast16_t>(from_fixed(result.origin.x - cacheOriginX_));
  int_fast16_t dstStartX = std::max<int_fast16_t>(0, srcOffsetX);
  int_fast16_t srcStartX = std::max<int_fast16_t>(0, -srcOffsetX);
  int_fast16_t copyWidth = std::min<int_fast16_t>(
      static_cast<int_fast16_t>(srcView.width) - srcStartX,
      cacheWidth_ - dstStartX);
  if (copyWidth > 0) {
    const uint8_t *srcPtr =
        static_cast<const uint8_t *>(srcView.data) + srcStartX * 4;
    std::memcpy(static_cast<uint8_t *>(dstView.data) + dstStartX * 4, srcPtr,
...
void VerticalBlurNode::fetchRowFromPrevStage(int_fast16_t stageIndex,
                                             Node *upstream,
                                             const RenderRequest &request,
                                             int_fast16_t srcY,
                                             int_fast16_t cacheIndex) {
  ...
  for (size_t x = 0; x < static_cast<size_t>(cacheWidth_); x++) {
    ...
    if (prevStage.colSumA[x] > 0) {
      dstRow[off] =
          static_cast<uint8_t>(prevStage.colSumR[x] / prevStage.colSumA[x]);
      dstRow[off + 1] =
          static_cast<uint8_t>(prevStage.colSumG[x] / prevStage.colSumA[x]);
      dstRow[off + 2] =
          static_cast<uint8_t>(prevStage.colSumB[x] / prevStage.colSumA[x]);
      dstRow[off + 3] = static_cast<uint8_t>(prevStage.colSumA[x] /
                                             static_cast<uint32_t>(ks));
      // 有効範囲を更新
      if (static_cast<int16_t>(x) < startX)
        startX = static_cast<int16_t>(x);
...
void VerticalBlurNode::updateStageColSum(BlurStage &stage,
                                         int_fast16_t cacheIndex, bool add) {
  const uint8_t *row = static_cast<const uint8_t *>(
      stage.rowCache[static_cast<size_t>(cacheIndex)].view().data);
...
void VerticalBlurNode::computeStageOutputRow(BlurStage &stage,
                                             ImageBuffer &output,
                                             int_fast16_t width) {
  uint8_t *outRow = static_cast<uint8_t *>(output.view().data);
  int_fast16_t ks = kernelSize();
  for (size_t x = 0; x < static_cast<size_t>(width); x++) {
    ...
    if (stage.colSumA[x] > 0) {
      ...
      outRow[off + 1] =
          static_cast<uint8_t>(stage.colSumG[x] / stage.colSumA[x]);
      outRow[off + 2] =
          static_cast<uint8_t>(stage.colSumB[x] / stage.colSumA[x]);
      outRow[off + 3] =
          static_cast<uint8_t>(stage.colSumA[x] / static_cast<uint32_t>(ks));
    } else {
  ...
  for (int_fast16_t s = 1; s < passes_; s++) {
    ...
    for (size_t x = 0; x < static_cast<size_t>(cacheWidth_); x++) {
      ...
      if (prevStage.colSumA[x] > 0) {
        ...
        stageRow[off + 2] =
            static_cast<uint8_t>(prevStage.colSumB[x] / prevStage.colSumA[x]);
        stageRow[off + 3] = static_cast<uint8_t>(prevStage.colSumA[x] /
                                                 static_cast<uint32_t>(ks));
      } else {
        stageRow[off] = stageRow[off + 1] = stageRow[off + 2] =
            stageRow[off + 3] = 0;
      }
    }
    ...
    std::memcpy(dstView.data, srcView.data,
                static_cast<size_t>(cacheWidth_) * 4);
...
void VerticalBlurNode::emitBlurredLinePipeline() {
  ...
  for (size_t x = 0; x < static_cast<size_t>(cacheWidth_); x++) {
    ...
    if (lastStage.colSumA[x] > 0) {
      outRow[off] =
          static_cast<uint8_t>(lastStage.colSumR[x] / lastStage.colSumA[x]);
      outRow[off + 1] =
          static_cast<uint8_t>(lastStage.colSumG[x] / lastStage.colSumA[x]);
      outRow[off + 2] =
          static_cast<uint8_t>(lastStage.colSumB[x] / lastStage.colSumA[x]);
      outRow[off + 3] = static_cast<uint8_t>(lastStage.colSumA[x] /
                                             static_cast<uint32_t>(ks));
    } else {
      outRow[off] = outRow[off + 1] = outRow[off + 2] = outRow[off + 3] = 0;
    }
...
void VerticalBlurNode::storeInputRowToStageCache(BlurStage &stage,
  ...
  int_fast16_t copyWidth =
      std::min<int_fast16_t>(srcWidth - srcStart, cacheWidth_ - dstStart);

  if (copyWidth > 0) {
    std::memcpy(dstData + dstStart * 4, srcData + srcStart * 4,
                static_cast<size_t>(copyWidth) * 4);
  }
}
