...
inline std::pair<int, int> calcValidRange(int_fixed coeff, int_fixed base, int srcSize, int canvasSize)
{
    ...
    if (coeff == 0) {
        ...
        return (srcIdx >= 0 && srcIdx < srcSize) ? std::make_pair(0, canvasSize - 1) : std::make_pair(1, 0);
    }
