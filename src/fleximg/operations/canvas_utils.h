...
inline void placeFirst(ViewPort &canvas, int_fixed canvasOriginX,
                       int_fixed canvasOriginY, const ViewPort &src,
                       int_fixed srcOriginX, int_fixed srcOriginY) {
  if (!canvas.isValid() || !src.isValid())
    return;

  // 新座標系: originはバッファ左上のワールド座標
  // srcをcanvasに配置する際のオフセット = src左端 - canvas左端
  auto offsetX =
      static_cast<int_fast16_t>(from_fixed(srcOriginX - canvasOriginX));
  auto offsetY =
      static_cast<int_fast16_t>(from_fixed(srcOriginY - canvasOriginY));
  ...
  auto copyWidth =
      std::min<int_fast16_t>(src.width - srcStartX, canvas.width - dstStartX);
  auto copyHeight =
      std::min<int_fast16_t>(src.height - srcStartY, canvas.height - dstStartY);
  ...
  if (canvas.formatID == PixelFormatIDs::RGBA8_Straight &&
      src.formatID->toStraight) {
    for (int_fast16_t y = 0; y < copyHeight; y++) {
      const void *srcRow = src.pixelAt(srcStartX, srcStartY + y);
      void *dstRow = canvas.pixelAt(dstStartX, dstStartY + y);
      src.formatID->toStraight(dstRow, srcRow, static_cast<size_t>(copyWidth),
                               nullptr);
    }
    return;
  }
  ...
  for (int y = 0; y < copyHeight; y++) {
    const void *srcRow = src.pixelAt(srcStartX, srcStartY + y);
    void *dstRow = canvas.pixelAt(dstStartX, dstStartY + y);
    convertFormat(srcRow, src.formatID, dstRow, canvas.formatID, copyWidth,
                  nullptr);
  }
}
