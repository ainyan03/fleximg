...
inline void calcAffineAABB(float inputWidth, float inputHeight,
                           Point inputOrigin, const AffineMatrix &matrix,
...
inline void calcInverseAffineAABB(int_fast16_t outputWidth,
                                  int_fast16_t outputHeight, Point outputOrigin,
                                  const AffineMatrix &matrix, int16_t &outWidth,
                                  int16_t &outHeight, Point &outOrigin) {
  // 逆行列を計算
  ...
  // 逆行列で変換
  calcAffineAABB(static_cast<float>(outputWidth),
                 static_cast<float>(outputHeight), outputOrigin, inv, outWidth,
                 outHeight, outOrigin);
}
...
struct RenderResponse {
  ...
  /// @brief バッファのビューを取得
  ViewPort view() { return entry_ ? entry_->buffer.view() : ViewPort(); }

  ViewPort view() const { return entry_ ? entry_->buffer.view() : ViewPort(); }
  ...
  ImageBuffer *createBuffer(int_fast16_t width, int_fast16_t height,
                            PixelFormatID format, InitPolicy policy) {
  ...
  void convertFormat(PixelFormatID format) {
    ...
    ImageBuffer converted(width, static_cast<int_fast16_t>(1), format,
    ...
    FLEXIMG_NAMESPACE::convertFormat(srcRow, srcFmt, dstRow, format, width,
                                     auxInfo);
