...
struct ViewPort {
  ...
  ViewPort(void *d, PixelFormatID fmt, int32_t str, int_fast16_t w,
           int_fast16_t h)
      : data(d), formatID(fmt), stride(str), width(static_cast<int16_t>(w)),
        height(static_cast<int16_t>(h)) {}

  // 簡易初期化（strideを自動計算）
  ViewPort(void *d, int_fast16_t w, int_fast16_t h,
           PixelFormatID fmt = PixelFormatIDs::RGBA8_Straight)
      : data(d), formatID(fmt),
        stride(static_cast<int32_t>(w * fmt->bytesPerPixel)),
        width(static_cast<int16_t>(w)), height(static_cast<int16_t>(h)) {}
  ...
  uint32_t rowBytes() const {
    return stride > 0 ? static_cast<uint32_t>(stride)
                      : static_cast<uint32_t>(width) *
                            static_cast<uint32_t>(bytesPerPixel());
  }
};
...
// 矩形クリア
void clear(ViewPort &dst, int_fast16_t x, int_fast16_t y, int_fast16_t width,
           int_fast16_t height);
...
void copyRowDDA(void *dst, const ViewPort &src, int_fast16_t count,
...
void copyRowDDABilinear(void *dst, const ViewPort &src, int_fast16_t count,
                        int_fixed srcX, int_fixed srcY, int_fixed incrX,
                        int_fixed incrY, uint8_t edgeFadeMask,
                        const PixelAuxInfo *srcAux);

// アフィン変換転写（DDA方式）
// 複数行を一括処理する高レベル関数
void affineTransform(ViewPort &dst, const ViewPort &src, int_fixed invTx,
                     int_fixed invTy, const Matrix2x2_fixed &invMatrix,
...
inline bool canUseSingleChannelBilinear(PixelFormatID formatID,
                                        uint8_t edgeFadeMask) {
  if (!formatID)
    return false;
  const int bytesPerPixel = formatID->bytesPerPixel;
  return (bytesPerPixel == 1) && (formatID->channelCount == 1) &&
         !formatID->isIndexed && (edgeFadeMask == 0 || formatID->hasAlpha);
}
...
void copy(ViewPort &dst, int_fast16_t dstX, int_fast16_t dstY,
  ...
  width = std::min<int_fast16_t>(
      width, std::min<int_fast16_t>(src.width - srcX, dst.width - dstX));
  height = std::min<int_fast16_t>(
      height, std::min<int_fast16_t>(src.height - srcY, dst.height - dstY));
  if (width <= 0 || height <= 0)
    return;
  ...
  for (int_fast16_t y = 0; y < height; ++y) {
    const uint8_t *srcRow =
        static_cast<const uint8_t *>(src.pixelAt(srcX, srcY + y));
...
__attribute__((noinline)) static void bilinearBlend_RGBA8888(
    uint32_t *__restrict__ dst, const uint32_t *__restrict__ quadPixels,
  ...
  for (int i = 0; i < count; ++i) {
    ...
    uint32_t f = static_cast<uint32_t>(fx) *
                 ((256 - fy) | (static_cast<uint32_t>(fy) << 16));
...
bilinearBlend_1ch(uint8_t *__restrict__ dst,
...
void copyRowDDABilinear(void *dst, const ViewPort &src, int_fast16_t count,
  ...
  if (canUseSingleChannelBilinear(src.formatID, edgeFadeMask)) {
    ...
    for (int_fast16_t offset = 0; offset < count; offset += CHUNK_SIZE) {
      int_fast16_t chunk =
          (count - offset < CHUNK_SIZE) ? (count - offset) : CHUNK_SIZE;
  ...
  const int srcBytesPerPixel =
      (src.formatID->pixelsPerUnit > 1)
  ...
  PixelFormatID converterSrcFormat =
      (src.formatID->pixelsPerUnit > 1) ? PixelFormatIDs::Index8 : src.formatID;
  if (converterSrcFormat != PixelFormatIDs::RGBA8_Straight) {
    converter = resolveConverter(converterSrcFormat,
                                 PixelFormatIDs::RGBA8_Straight, srcAux);
  }
  ...
  for (int_fast16_t offset = 0; offset < count; offset += CHUNK_SIZE) {
    ...
    auto quadPtr =
        reinterpret_cast<uint8_t *>(quadBuffer) + (dstQuadSize - srcQuadSize);
...
void affineTransform(ViewPort &dst, const ViewPort &src, int_fixed invTx,
  ...
  for (int dy = 0; dy < outH; dy++) {
    ...
    auto [xStart, xEnd] =
        transform::calcValidRange(incrX, rowBaseX, src.width, outW);
    auto [yStart, yEnd] =
        transform::calcValidRange(incrY, rowBaseY, src.height, outW);
