...
public:
  ...
  ImageBuffer()
      : view_(), capacity_(0),
        allocator_(&core::memory::DefaultAllocator::instance()), auxInfo_(),
        origin_(), initPolicy_(DefaultInitPolicy) {}

  // サイズ指定コンストラクタ
  // alloc = nullptr の場合、DefaultAllocator を使用
  ImageBuffer(int_fast16_t w, int_fast16_t h,
              PixelFormatID fmt = PixelFormatIDs::RGBA8_Straight,
              InitPolicy init = DefaultInitPolicy,
              core::memory::IAllocator *alloc = nullptr)
      : view_(nullptr, fmt, 0, static_cast<int16_t>(w),
  ...
  ImageBuffer(const ImageBuffer &other)
      : view_(nullptr, other.view_.formatID, 0, other.view_.width,
              other.view_.height),
        capacity_(0),
        allocator_(other.allocator_
                       ? other.allocator_
                       : &core::memory::DefaultAllocator::instance()),
        auxInfo_(other.auxInfo_), origin_(other.origin_),
  ...
  ViewPort subView(int_fast16_t x, int_fast16_t y, int_fast16_t w,
                   int_fast16_t h) const {
    return view_ops::subView(view_, x, y, w, h);
  }

  // サブビューを持つ参照モードImageBufferを作成
  ImageBuffer subBuffer(int_fast16_t x, int_fast16_t y, int_fast16_t w,
                        int_fast16_t h) const {
  ...
  void cropView(int_fast16_t x, int_fast16_t y, int_fast16_t w,
                int_fast16_t h) {
  ...
  uint32_t totalBytes() const {
    ...
    return static_cast<uint32_t>(view_.height) *
           static_cast<uint32_t>(absStride);
  }
  ...
  ImageBuffer toFormat(PixelFormatID target,
    ...
    if (view_.formatID == target) {
      ...
      if (isValid() && copied.isValid()) {
        view_ops::copy(copied.view_, 0, 0, view_, 0, 0, view_.width,
                       view_.height);
      }
      return copied;
    }
...
private:
  ...
  void allocate() {
    ...
    if (view_.formatID->pixelsPerUnit > 1) {
      ...
      int units = (view_.width + view_.formatID->pixelsPerUnit - 1) /
                  view_.formatID->pixelsPerUnit;
    ...
    capacity_ =
        static_cast<size_t>(view_.stride) * static_cast<size_t>(view_.height);
    if (capacity_ > 0 && allocator_) {
      ...
      if (view_.data) {
        ...
        PerfMetrics::instance().recordAlloc(capacity_, view_.width,
                                            view_.height);
#endif
      }
    }
  ...
  void copyFrom(const ImageBuffer &other) {
    ...
    for (int_fast16_t y = 0; y < copyHeight; ++y) {
      // ViewPortのx,yオフセットを考慮
      std::memcpy(static_cast<uint8_t *>(view_.data) +
                      (view_.y + y) * view_.stride +
                      view_.x * view_.bytesPerPixel(),
...
inline bool ImageBuffer::blendFrom(const ImageBuffer &src) {
  if (!isValid() || !src.isValid() || !view_.data)
    return false;
  ...
  if (remaining <= 0)
    return true; // 範囲外、何もしない

  const size_t dstPixelBytes =
      static_cast<size_t>(view_.formatID->bytesPerPixel);
  ...
  const uint8_t *srcRowBase =
      static_cast<const uint8_t *>(srcView.data) + srcView.y * srcView.stride;

  PixelFormatID srcFmt = srcView.formatID;
  const PixelAuxInfo *srcAux = &src.auxInfo();

  // 共通: クリッピング開始位置のポインタ計算
  const int_fast32_t srcTotalBits =
      (srcView.x + clippedStart - srcStartX) * srcPixelBits;
  const void *srcPtr = srcRowBase + static_cast<size_t>(srcTotalBits >> 3);
  void *dstPtr =
      dstRow + static_cast<size_t>(clippedStart - dstStartX) * dstPixelBytes;
  ...
  if (blendFunc) {
    ...
    auto converter =
        resolveConverter(srcFmt, PixelFormatIDs::RGBA8_Straight, srcAux);
    if (!converter)
      return false;
    converter.ctx.pixelOffsetInByte =
        static_cast<uint8_t>((srcTotalBits & 7) >> (srcPixelBits >> 1));
    ...
    const size_t srcBytesPerChunk =
        static_cast<size_t>(CHUNK_SIZE * srcPixelBits >> 3);

    do {
      int_fast16_t chunk = std::min(remaining, CHUNK_SIZE);
      converter(tempBuf, srcChunkPtr, chunk);
      void *dstChunkPtr =
          dstRow + static_cast<size_t>(cursor - dstStartX) * dstPixelBytes;
