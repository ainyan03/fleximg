...
template <> struct PixelType<1> {
  using type = uint8_t;
};
template <> struct PixelType<2> {
  using type = uint16_t;
};
template <> struct PixelType<4> {
  using type = uint32_t;
};
...
void copyRowDDA_ConstY(uint8_t *__restrict__ dstRow,
                       const uint8_t *__restrict__ srcData, int_fast16_t count,
                       const DDAParam *param) {
  int_fixed srcX = param->srcX;
  const int_fixed incrX = param->incrX;
  const int32_t srcStride = param->srcStride;
  const uint8_t *srcRowBase =
      srcData +
      static_cast<size_t>((param->srcY >> INT_FIXED_SHIFT) * srcStride);
  ...
  if constexpr (BytesPerPixel == 3) {
    if (count & 1) {
      ...
      uint8_t p00 = srcRowBase[s0], p01 = srcRowBase[s0 + 1],
              p02 = srcRowBase[s0 + 2];
      srcX += incrX;
      dstRow[0] = p00;
      dstRow[1] = p01;
    ...
    while (count--) {
      ...
      uint8_t p10 = srcRowBase[s1], p11 = srcRowBase[s1 + 1],
              p12 = srcRowBase[s1 + 2];
      srcX += incrX;
      dstRow[3] = p10;
      dstRow[4] = p11;
...
// 4ピクセル単位展開でループオーバーヘッドを削減
template <size_t BytesPerPixel>
void copyRowDDA_ConstX(uint8_t *__restrict__ dstRow,
                       const uint8_t *__restrict__ srcData, int_fast16_t count,
                       const DDAParam *param) {
  ...
  const uint8_t *srcColBase =
      srcData + static_cast<size_t>((param->srcX >> INT_FIXED_SHIFT) *
  ...
  if constexpr (BytesPerPixel == 3) {
    ...
    while (remain--) {
      ...
      auto p = *reinterpret_cast<const T *>(
          srcColBase + static_cast<size_t>(sy * srcStride));
      srcY += incrY;
      dst[0] = p;
      dst += 1;
    }
    ...
    while (count--) {
      // BytesPerPixel 1, 2, 4: ネイティブ型でロード・ストア分離
      sy = srcY >> INT_FIXED_SHIFT;
      auto p0 = *reinterpret_cast<const T *>(
          srcColBase + static_cast<size_t>(sy * srcStride));
      srcY += incrY;
      sy = srcY >> INT_FIXED_SHIFT;
      auto p1 = *reinterpret_cast<const T *>(
          srcColBase + static_cast<size_t>(sy * srcStride));
      srcY += incrY;
      dst[0] = p0;
      dst[1] = p1;
      sy = srcY >> INT_FIXED_SHIFT;
      auto p2 = *reinterpret_cast<const T *>(
          srcColBase + static_cast<size_t>(sy * srcStride));
      srcY += incrY;
      sy = srcY >> INT_FIXED_SHIFT;
      auto p3 = *reinterpret_cast<const T *>(
          srcColBase + static_cast<size_t>(sy * srcStride));
      srcY += incrY;
      dst[2] = p2;
      dst[3] = p3;
...
// 4ピクセル単位展開でループオーバーヘッドを削減
template <size_t BytesPerPixel>
void copyRowDDA_Impl(uint8_t *__restrict__ dstRow,
                     const uint8_t *__restrict__ srcData, int_fast16_t count,
                     const DDAParam *param) {
  int_fixed srcY = param->srcY;
  int_fixed srcX = param->srcX;
  ...
  if constexpr (BytesPerPixel == 3) {
    while (count--) {
      ...
      sy = srcY >> INT_FIXED_SHIFT;
      const uint8_t *r0 =
          srcData + static_cast<size_t>(sy * srcStride + sx * 3);
    ...
    if (count & 1) {
      ...
      sy = srcY >> INT_FIXED_SHIFT;
      auto p = reinterpret_cast<const T *>(
          srcData + static_cast<size_t>(sy * srcStride))[sx];
      srcX += incrX;
      srcY += incrY;
      d[0] = p;
      d++;
...
template <size_t BytesPerPixel>
void copyRowDDA_Byte(uint8_t *dst, const uint8_t *srcData, int_fast16_t count,
                     const DDAParam *param) {
  const int_fixed srcY = param->srcY;
  const int_fixed incrY = param->incrY;
  // ソース座標の整数部が全ピクセルで同一か判定（座標は呼び出し側で非負が保証済み）
  if (0 == (((srcY & ((1 << INT_FIXED_SHIFT) - 1)) + incrY * count) >>
            INT_FIXED_SHIFT)) {
  ...
  const int_fixed incrX = param->incrX;
  if (0 == (((srcX & ((1 << INT_FIXED_SHIFT) - 1)) + incrX * count) >>
            INT_FIXED_SHIFT)) {
    // X座標一定パス（微小X変動も含む）
...
template void copyRowDDA_Byte<1>(uint8_t *, const uint8_t *, int_fast16_t,
                                 const DDAParam *);
template void copyRowDDA_Byte<2>(uint8_t *, const uint8_t *, int_fast16_t,
                                 const DDAParam *);
template void copyRowDDA_Byte<3>(uint8_t *, const uint8_t *, int_fast16_t,
                                 const DDAParam *);
template void copyRowDDA_Byte<4>(uint8_t *, const uint8_t *, int_fast16_t,
                                 const DDAParam *);

// BytesPerPixel別の関数ポインタ取得用ラッパー（非テンプレート）
inline void copyRowDDA_1Byte(uint8_t *dst, const uint8_t *srcData,
                             int_fast16_t count, const DDAParam *param) {
  copyRowDDA_Byte<1>(dst, srcData, count, param);
}
inline void copyRowDDA_2Byte(uint8_t *dst, const uint8_t *srcData,
                             int_fast16_t count, const DDAParam *param) {
  copyRowDDA_Byte<2>(dst, srcData, count, param);
}
inline void copyRowDDA_3Byte(uint8_t *dst, const uint8_t *srcData,
                             int_fast16_t count, const DDAParam *param) {
  copyRowDDA_Byte<3>(dst, srcData, count, param);
}
inline void copyRowDDA_4Byte(uint8_t *dst, const uint8_t *srcData,
                             int_fast16_t count, const DDAParam *param) {
...
template <size_t BytesPerPixel>
inline void copyQuadPixels(uint8_t *__restrict__ dst, const uint8_t *p00,
                           const uint8_t *p10, const uint8_t *p01,
                           const uint8_t *p11) {
  if constexpr (BytesPerPixel == 3) {
    dst[0] = p00[0];
    dst[1] = p00[1];
...
template <size_t BytesPerPixel>
void copyQuadDDA_Byte(uint8_t *__restrict__ dst,
                      const uint8_t *__restrict__ srcData, int_fast16_t count,
                      const DDAParam *param) {
  ...
  for (int_fast16_t i = 0; i < count; ++i) {
    ...
    weightsXY[i].fx = static_cast<uint8_t>(static_cast<uint32_t>(srcX) >>
                                           (INT_FIXED_SHIFT - 8));
    weightsXY[i].fy = static_cast<uint8_t>(static_cast<uint32_t>(srcY) >>
                                           (INT_FIXED_SHIFT - 8));
    srcX += incrX;
    srcY += incrY;
...
// 明示的インスタンス化
template void copyQuadDDA_Byte<1>(uint8_t *, const uint8_t *, int_fast16_t,
                                  const DDAParam *);
template void copyQuadDDA_Byte<2>(uint8_t *, const uint8_t *, int_fast16_t,
                                  const DDAParam *);
template void copyQuadDDA_Byte<3>(uint8_t *, const uint8_t *, int_fast16_t,
                                  const DDAParam *);
template void copyQuadDDA_Byte<4>(uint8_t *, const uint8_t *, int_fast16_t,
                                  const DDAParam *);

// BytesPerPixel別の関数ポインタ取得用ラッパー（非テンプレート）
inline void copyQuadDDA_1Byte(uint8_t *dst, const uint8_t *srcData,
                              int_fast16_t count, const DDAParam *param) {
  copyQuadDDA_Byte<1>(dst, srcData, count, param);
}
inline void copyQuadDDA_2Byte(uint8_t *dst, const uint8_t *srcData,
                              int_fast16_t count, const DDAParam *param) {
  copyQuadDDA_Byte<2>(dst, srcData, count, param);
}
inline void copyQuadDDA_3Byte(uint8_t *dst, const uint8_t *srcData,
                              int_fast16_t count, const DDAParam *param) {
  copyQuadDDA_Byte<3>(dst, srcData, count, param);
}
inline void copyQuadDDA_4Byte(uint8_t *dst, const uint8_t *srcData,
                              int_fast16_t count, const DDAParam *param) {
...
void copyRowDDA_Bit_ConstY(uint8_t *__restrict__ dst,
  ...
  const uint8_t *srcRow =
      srcData + static_cast<size_t>(sy) * static_cast<size_t>(param->srcStride);
...
template <int BitsPerPixel, BitOrder Order>
inline void copyRowDDA_Bit(uint8_t *dst, const uint8_t *srcData,
                           int_fast16_t count, const DDAParam *param) {
  const int_fixed srcY = param->srcY;
  const int_fixed incrY = param->incrY;
  ...
  for (int_fast16_t i = 0; i < count; ++i) {
    ...
    if (param->srcWidth > 0 && param->srcHeight > 0) {
      FLEXIMG_REQUIRE(sx >= 0 && sx < param->srcWidth && sy >= 0 &&
                          sy < param->srcHeight,
...
template <int BitsPerPixel, BitOrder Order>
inline void copyQuadDDA_Bit(uint8_t *dst, const uint8_t *srcData,
                            int_fast16_t count, const DDAParam *param) {
  ...
  for (int_fast16_t i = 0; i < count; ++i) {
    ...
    if (x_valid && y_valid) {
      // 全て範囲内: 2x2グリッドを読み取り
      dst[0] = bit_packed_detail::readPixelDirect<BitsPerPixel, Order>(
          srcData, sx, sy, srcStride);
      dst[1] = bit_packed_detail::readPixelDirect<BitsPerPixel, Order>(
          srcData, sx + 1, sy, srcStride);
      dst[2] = bit_packed_detail::readPixelDirect<BitsPerPixel, Order>(
          srcData, sx, sy + 1, srcStride);
      dst[3] = bit_packed_detail::readPixelDirect<BitsPerPixel, Order>(
          srcData, sx + 1, sy + 1, srcStride);
      if (edgeFlags)
        edgeFlags[i] = 0;
    } else {
