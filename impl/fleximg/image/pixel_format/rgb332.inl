/**
 * @file rgb332.inl
 * @brief RGB332 ピクセルフォーマット 実装
 * @see src/fleximg/image/pixel_format/rgb332.h
 */
...
#define RGB332_ENTRY(p)                                                                   \
    static_cast<uint32_t>((static_cast<uint32_t>((((p) >> 5) & 0x07) * 0x49 >> 1)) |      \
                          (static_cast<uint32_t>((((p) >> 2) & 0x07) * 0x49 >> 1) << 8) | \
                          (static_cast<uint32_t>(((p) & 0x03) * 0x55) << 16) | (static_cast<uint32_t>(255) << 24))
...
// RGB332 -> RGBA8 変換テーブル (256 x 4 = 1024 bytes)
alignas(64) static const uint32_t rgb332ToRgba8[256] = {
    RGB332_ROW(0x00), RGB332_ROW(0x10), RGB332_ROW(0x20), RGB332_ROW(0x30), RGB332_ROW(0x40), RGB332_ROW(0x50),
    RGB332_ROW(0x60), RGB332_ROW(0x70), RGB332_ROW(0x80), RGB332_ROW(0x90), RGB332_ROW(0xa0), RGB332_ROW(0xb0),
...
static void rgb332_toStraight(void *dst, const void *src, size_t pixelCount, const PixelAuxInfo *)
{
    ...
    pixel_format::detail::lut8to32(static_cast<uint32_t *>(dst), static_cast<const uint8_t *>(src), pixelCount,
                                   rgb332ToRgba8);
}

// RGBA8 -> RGB332 変換マクロ（32bitロードした値から変換）
// (((r << 3) + g) << 2) + b の形式でESP32のシフト+加算命令を活用
#define RGBA8_TO_RGB332(rgba) (((((rgba) >> 5) << 3) + (((rgba) >> 13) & 0x07)) << 2) + (((rgba) >> 22) & 0x03)
