...
static inline void applyColorKey(uint32_t *rgba8, size_t pixelCount, uint32_t colorKey, uint32_t replace)
{
...
// 同一フォーマット: memcpy
static void fcv_memcpy(void *dst, const void *src, size_t pixelCount, const void *ctx)
{
...
static void fcv_single(void *dst, const void *src, size_t pixelCount, const void *ctx)
{
    auto *c = static_cast<const FormatConverter::Context *>(ctx);
    c->toStraight(dst, src, pixelCount, nullptr);
    applyColorKey(static_cast<uint32_t *>(dst), pixelCount, c->colorKeyRGBA8, c->colorKeyReplace);
}

// Index展開: パレットフォーマット == 出力フォーマット（直接展開）
static void fcv_expandIndex_direct(void *dst, const void *src, size_t pixelCount, const void *ctx)
{
    auto *c = static_cast<const FormatConverter::Context *>(ctx);
    PixelAuxInfo aux;
...
// チャンク処理でアロケーション不要
static void fcv_expandIndex_fromStraight(void *dst, const void *src, size_t pixelCount, const void *ctx)
{
    auto *c = static_cast<const FormatConverter::Context *>(ctx);
    ...
    while (remaining > 0) {
        ...
        applyColorKey(reinterpret_cast<uint32_t *>(straightBuf), chunk, c->colorKeyRGBA8, c->colorKeyReplace);
...
static void fcv_expandIndex_toStraight_fromStraight(void *dst, const void *src, size_t pixelCount, const void *ctx)
{
    auto *c = static_cast<const FormatConverter::Context *>(ctx);
    FLEXIMG_ASSERT(c->paletteBytesPerPixel <= MAX_BYTES_PER_PIXEL, "paletteBytesPerPixel exceeds MAX_BYTES_PER_PIXEL");
    ...
    while (remaining > 0) {
        ...
        applyColorKey(reinterpret_cast<uint32_t *>(buf), chunk, c->colorKeyRGBA8, c->colorKeyReplace);
...
// チャンク処理でアロケーション不要
static void fcv_toStraight_fromStraight(void *dst, const void *src, size_t pixelCount, const void *ctx)
{
    auto *c = static_cast<const FormatConverter::Context *>(ctx);
    uint8_t straightBuf[FCV_CHUNK_SIZE * MAX_BYTES_PER_PIXEL];
...
FormatConverter resolveConverter(PixelFormatID srcFormat, PixelFormatID dstFormat, const PixelAuxInfo *srcAux)
{
    ...
    if (dstFormat == PixelFormatIDs::RGBA8_Straight) {
        if (srcFormat->toStraight) {
            ...
            if (srcAux && !srcFormat->hasAlpha && srcAux->colorKeyRGBA8 != srcAux->colorKeyReplace) {
                result.ctx.colorKeyRGBA8   = srcAux->colorKeyRGBA8;
                result.ctx.colorKeyReplace = srcAux->colorKeyReplace;
            }
            result.func = fcv_single;
