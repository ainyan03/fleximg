 ...
 * @file sink_node.inl
 * @brief SinkNode 実装
 * @see src/fleximg/nodes/sink_node.h
 */
...
PrepareResponse SinkNode::onPushPrepare(const PrepareRequest &request)
{
    ...
    if (request.hasPushAffine || hasLocalTransform()) {
        ...
        if (invMatrix_.valid) {
            ...
            int64_t invCombinedX = (combinedX * invMatrix_.a + combinedY * invMatrix_.b) >> INT_FIXED_SHIFT;
            int64_t invCombinedY = (combinedX * invMatrix_.c + combinedY * invMatrix_.d) >> INT_FIXED_SHIFT;
    ...
    if (hasTransform && invMatrix_.valid) {
        ...
        calcInverseAffineAABB(target_.width, target_.height, {combinedPivotX, combinedPivotY}, aabbMatrix, result.width,
                              result.height, result.origin);
    } else {
        // アフィンなしの場合
        ...
        result.origin = {-float_to_fixed(localMatrix_.tx) - pivotX_, -float_to_fixed(localMatrix_.ty) - pivotY_};
    }
    return result;
}
...
void SinkNode::onPushProcess(RenderResponse &input, const RenderRequest &request)
{
    ...
    auto converter = resolveConverter(inputView.formatID, target_.formatID, &input.buffer().auxInfo());
...
void SinkNode::applyAffine(ViewPort &dst, const ViewPort &src, int_fixed srcOriginX, int_fixed srcOriginY)
{
    ...
    // 共通DDA処理を呼び出し
    view_ops::affineTransform(dst, src, fixedTx, fixedTy, invMatrix_, rowOffsetX, rowOffsetY, dxOffsetX, dxOffsetY);
}
